...
class Gsheet():
    ...
    def __init__(self):
        ...
        self._primary_keys_headers = self.__gsheet_settings['primary_keys_headers']
        # ---------------------------------------------------------------------------
    ...
    @property
    def input_headers_sanity_check(self):
        '''
        Property to get dataframe from google sheet
        '''
        df_input = self.__read_data_from_sheet_to_dataframe()
        df_input_headers = [header.lower() for header in df_input.columns.tolist()]
        for primary_key in self._primary_keys_headers:
            if not primary_key.lower() in df_input_headers:
                return False
        return True
